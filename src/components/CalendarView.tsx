...
const CalendarView: React.FC<CalendarViewProps> = ({ games }) => {
  ...
  const yearOptions = Array.from({ length: 41 }, (_, index) => currentYear - 20 + index);

  return (
    <div className="flex flex-col h-full min-h-screen bg-gray-50 text-gray-800 font-sans">
      {/* ヘッダー */}
      <header className="bg-white shadow-sm border-b px-6 py-4 flex flex-col gap-3 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center bg-gray-100 rounded-lg p-1">
              ...
              <select
                ...
                className="mx-1 rounded-md border border-gray-200 bg-white px-2 py-1.5 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                aria-label="年を選択"
              >
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-gray-600">
          <div className="flex items-center gap-1.5">
            <span className="inline-block h-2.5 w-2.5 rounded-full border border-yellow-400 bg-yellow-200"></span>
            <span>黄色 = アニバーサリー</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="inline-block h-2.5 w-2.5 rounded-full border border-blue-400 bg-blue-200"></span>
            <span>青色 = ハーフアニバーサリー</span>
          </div>
        </div>
      </header>
          ...
          <div className="grid grid-cols-7 auto-rows-fr bg-gray-200 gap-px border-b border-l border-r border-gray-200">
            {calendarDays.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className={`min-h-[140px] p-2 bg-white transition-colors
                ...
                >
                  ...
                  <div className="space-y-1.5 mt-1.5 max-w-full">
                    {events.map((event, idx) => (
                      <div
                        key={`${event.gameId}-${idx}`}
                        className={`
                          px-1 py-0.5 text-[10px] sm:text-[11px] font-semibold rounded-md shadow-sm border leading-tight
                          ...
                          overflow-hidden transition-transform hover:scale-[1.02] cursor-default
                        `}
                        title={`${event.title} ${event.label}`}
                      >
                        <span className="block break-words leading-snug text-[10px] sm:text-[11px]">
                          {event.label}
                        </span>
                      </div>
                    ))}
      ...
      <footer className="px-4 pb-5 pt-3 text-center text-[11px] text-gray-500">
        ※実際にキャンペーンを行っているかは調べておりません。アニバーサリーとハーフアニバーサリーを算出しているだけです。
      </footer>
    </div>
  );
};
